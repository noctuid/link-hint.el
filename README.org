...
* About
=link-hint.el= is inspired by the link hinting functionality in vim-like browsers and browser plugins such as pentadactyl. It provides commands for using avy (or ivy or another completion framework) to open, copy, or take a user-defined action on "links."
...
- link-hint supports selecting links with completion frameworks like ivy, helm, selectrum, etc.
...
These commands are all just short wrappers around ~link-hint-avy-select~. If you want a specific behavior that isn't provided by one of these commands, you can define your own command using it.
...
** Selecting Actions
link-hint allows selecting actions before or after selecting links. You can select the action to take in the following ways:
- Select the action beforehand by using a specific command/keybinding (e.g. bind a key to ~link-hint-open~)
- Select the action with =link-hint-action-completing-read-function= before or after selecting a link
  - For example, bind a key directly to ~link-hint-avy-select~ to select the action before
  - For example, bind a key to a command that calls ~(link-hint-avy-select :prompt-after t)~ to select the action after selecting a link
- Select an action using =avy-dispatch-alist=, =ivy-set-actions=, or some other framework-specific method

Deciding on the action beforehand has the advantage of narrowing the valid links to only those that support the action. By default, link-hint uses ~completing-read~ for action selection (e.g. will use ivy if ~ivy-mode~ is enabled). If you wanted to use avy, for example, you could set it to ~frog-menu-completing-read-function~ from [[https://github.com/clemera/frog-menu][frog-menu]].

Note that if you use a framework-specific method for selecting the action, it is possible to select an invalid action for a given link. If you select the action after the link with the builtin method, link-hint will only show valid actions. If you use =avy-dispatch-alist= or ~~ivy-dispatching-done~, then there is no validation, and it is up to you to select a valid action.

Here is an example of how to use =avy-dispatch-alist= to set an action:
#+begin_src emacs-lisp
(defun my-link-hint-avy-select ()
  (interactive)
  (let ((avy-dispatch-alist
         ;; bind . to set the action to :copy
         (list (cons ?\. (lambda (pt)
                           (link-hint-set-action :copy)
                           pt)))))
    ;; default to :open
    (link-hint-avy-select :action :open :prompt-after t)))
#+end_src

Here is an example of how to use ivy actions to select an action if you use ~ivy-mode~:
#+begin_src emacs-lisp
(defun my-link-hint-completing-select ()
  (interactive)
  (link-hint-completing-select :action :open :prompt-after t))

(ivy-set-actions
 'my-link-hint-completing-select
 '(("c" (lambda (_) (link-hint-set-action :copy)) "copy link")))
#+end_src

** Selecting Links with Helm, Ivy, Selectrum, etc.
If you want to select from all links in the current buffer with ivy or another completion framework, you can use ~link-hint-completing-select~. If bound directly to a key, it will prompt for an action and then allow selecting any link in the current buffer. It uses =link-hint-link-completing-read-function= for selecting a single link or =link-hint-link-completing-read-multiple-function= for selecting multiple links. These default to ~completing-read~ and ~completing-read-multiple~ respectively. If you have ~ivy-mode~ (supports ~completing-read~), ~selectrum-mode~ (supports both ~completing-read~ and ~completing-read-multiple~), etc. enabled, you do not need to explicitly set these variables.

~link-hint-completing-select~ also supports selecting only from visible links or from multiple windows (based on =(link-)hint-avy-all-windows= or whether it is passed =:all-windows <arg>=). For more information, see its docstring. link-hint does not provide any other commands since there are a large number of possible combinations. You can create more specific yourself commands if you need to:
#+begin_src emacs-lisp
(defun my-link-hint-completing-open ()
  (interactive)
  (link-hint-completing-select :action :open))

(defun my-link-hint-completing-open-multiple ()
  (interactive)
  (link-hint-completing-select :action :open :multiple t))
#+end_src

** Defining New Link Types and Actions
~link-hint-define-type~ is the helper function used to define new link types. ~link-hint-define-type~ is just simple helper to alter the symbol plist of =link-hint-<type>= (though you should always use it directly in case the implementation changes; it now does some extra work). For example, here is how =shr-url= could be defined if it did not already exist:
#+begin_src emacs-lisp
