...
By default, these variables are not bound, and avy's corresponding settings are used.
...
If moving to the link involves changing the window or point position, link-hint will automatically restore both when it makes sense to. For example, if the action opens a new window, the old window will not be restored. If a local org link (for another heading in the same file) is followed, the old position will not be restored.
...
#+end_src
This may not be all that useful (could alternatively just collect using a next url function)
